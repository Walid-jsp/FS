...
class Game {
    ...
    createScene() {
        ...
        SceneLoader.ImportMeshAsync("", "", labo, scene).then((result) => {
            // Récupérer le premier mesh (dungeonMesh)
            let laboMesh = result.meshes[0];
        
            // Appliquer les transformations (scaling, position)
            laboMesh.scaling = new Vector3(3,3,3);
            laboMesh.position = new Vector3(0, 0, 0);
        
            // Activer les collisions et les ombres
            laboMesh.checkCollisions = true;
            laboMesh.receiveShadows = true;
        
            // Appliquer la physique et les propriétés de chaque mesh enfant
            for (let childMesh of result.meshes) {
                childMesh.refreshBoundingInfo(true); // Mise à jour des informations de bounding box
        
                if (childMesh.getTotalVertices() > 0) {
                    // Créer un agrégat physique pour chaque mesh
                    const meshAggregate = new PhysicsAggregate(childMesh, PhysicsShapeType.MESH, { mass: 0, friction: 0.4, restitution: 0.1 }, scene);
                    
                    // Définir le type de mouvement pour la physique (statique)
                    meshAggregate.body.setMotionType(PhysicsMotionType.STATIC);
                    
                    // Activer les ombres pour chaque enfant
        ...
        SceneLoader.ImportMeshAsync("", "", portail, scene).then((result) => {
            ...
            portailMesh.position = new Vector3(
                playerPos.x,
                2,                    // <-- Position verticale corrigée ici
                playerPos.z + 5         // un peu devant le joueur
            );
